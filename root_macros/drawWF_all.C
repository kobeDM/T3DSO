 ...
 * Usage: $root 'drawWF_all.C("filename",maxevnum)'
 * Author K. Miuchi
...
int drawWF_all(string filename, const Int_t  maxevents){
  ...
  for ( int ch = 0; ch < chN; ch++ ) {     
    ...
    histArr_LE[ch]= new TH2D( Form( "hist_LE%d", ch+1 ), Form( "%s CH %d (zoomed);clock;adc",filename.c_str(), ch+1 ), 150, 400, 600, 128, 0., 128. ) ;
   ...
   if(TDIV_char[TDIV_char_length]=='n'){
	...
	for(int i=0;i<length[ch];i++){
	  ...
	  //if(wf[1][0][i]>255)
	  if(wf[1][ch][i]>255)
	   if(ev==0) cout<<dec<<" "<<index<< " "<<wf[1][ch][i]<<endl;
	  index++;
	}
	//if(ev==0){
	if(length[0]>0){
  ...
  tree->Fill();
    ...
    //    cout<<"\n";  
    cout<<dec<<" data end check (should be 0xa 0xa):"<<endl;
    for(int i=0;i<2;i++){
